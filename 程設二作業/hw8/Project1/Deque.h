...
//      return ( ( myData.off / 4 ) & ( myData.mapSize - 1 ) );
   ...
   void doubleMapSize()
   {
      if( myData.mapSize > 0 )
      {
         Ty **newMap = new Ty*[ 2 * myData.mapSize ]();  //新增2倍的1格陣列

         size_type i = myData.myOff / 4;
         for( ; i < ( myData.myOff + myData.mySize +3 ) / 4; i++ )   //將會用到的4格陣列實體化
            newMap[ i ] = new Ty[ 4 ]();
         ...
         {
            if( j == 4 * myData.mapSize )//為了讓超過mapsize的位置 的數字可以上下交換
               j = 0;
            newMap[ i / 4 ][ i % 4 ] = myData.map[ j / 4 ][ j % 4 ];  //上下對調數字
         }
         ...
         myData.map = newMap;   //將pointer指向新陣列
         myData.mapSize *= 2;
      }
   }
