...
#include "C:\Users\User\Desktop\基哥程設二作業\作業9\Vector.h"
#include "C:\Users\User\Desktop\基哥程設二作業\作業9\List.h"
...
HugeInteger< T > HugeInteger< T >::operator/( HugeInteger< T > &op2 )
{
   ...
   *(++ten.integer.begin()) = 1;
   ...
   typename  T::iterator it1 = --remainder.integer.end();
   typename  T::iterator it2 = --integer.end();
   typename  T::iterator it3 = --op2.integer.end();
   typename T::iterator it5 = --buffer.integer.end();

   HugeInteger  quotient(integer.size() - op2.integer.size());
   typename  T::iterator it6 = --quotient.integer.end();
   ...
   typename T::iterator it7 = quotient.integer.end();
   for (it6 = --quotient.integer.end(); it7 != (quotient.integer.begin());)  //做除法的計算
   {
       while (!(remainder < buffer))    //當buffer<=remainder
       {
           ...
           ++(*it6); //quotient的值
           it1 = --remainder.integer.end();
           while (*(it1) == 0 && remainder.integer.size() > 1)
           {
               --it1;
