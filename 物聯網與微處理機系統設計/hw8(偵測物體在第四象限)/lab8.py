    ...
    while(True):
        ...
        for i in range(int(num[0])):
            if sc[i] > 0.5:
                ...
                if x > 0.5 and y > 0.5:
                    frame = cv2.rectangle(frame, (int(IM_WIDTH / 2),int(IM_HEIGHT/2)),(int(IM_WIDTH-1),int(IM_HEIGHT-1)),(0,0,255),2)

                frame = cv2.circle(frame, (int(cx*IM_WIDTH),int(cy*IM_HEIGHT)), radius=5, color=(0,0,255), thickness=-1)
                #frame = cv2.rectangle(frame, (midX,midY),(midX*2,midY*2), color=(0,0,255),thickness = 2)
